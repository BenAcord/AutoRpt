...
def set_bugbounty_engagement():
    ...
    return_values = []
    # No template path.
    return_values = [platform, engagement_name, templates_path, platform_name, '', '']
    return return_values
...
def set_exam_engagement():
    ...
    if '' == student_id:
        out.color_notice('What is your student ID?')
        student_id = (str(input('>  ')))
    return_values = []
    # No template path.
    return_values = [platform, engagement_name, templates_path, platform_name, student_id]
    return return_values
...
def set_pentest_engagement():
    ...
    return_values = []
    # No template path.
    return_values = [platform, engagement_name, '', platform_name]
    return return_values
...
def set_default_template_path(engagement_type, student_name, student_email):
    ...
    copy_template(input_values[2], this_dir, engagement_type, input_values[1], input_values[5])
    ...
    this_msg = f'Startup initiated for {engagement_type} as {input_values[1]}'
    work.sitrep_auto(this_msg)
    this_msg = f'New working directory is {this_dir}'
    work.sitrep_auto(this_msg)
...
def copy_template(templates_path, this_dir, engagement_type, engagement_name, target_ip):
    ...
    try:
        shutil.copytree(templates_path, f'{this_dir}/')
    except (FileNotFoundError, PermissionError, IOError, OSError):
        out.color_fail("[!]", "Copytree templates failed.")
        try:
            shutil.copy(templates_path, f'{this_dir}/')
        except (FileNotFoundError, PermissionError, IOError, OSError):
            out.color_fail("[!]", "Copy templates failed.")
            sys.exit(5)
